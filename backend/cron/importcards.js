...
const processCards = async () => {
  ...
  for (const file of files) {
    if (!file.startsWith("products_singles_")) continue;

    const cm_game_id = parseInt(
      file.replace("products_singles_", "").replace(".json", ""),
    );
    ...
    for (const entry of data.products) {
      ...
      rows.push([entry.name, entry.idProduct, expansion_id]);
    }

    if (rows.length === 0) continue;

    await query(
      "INSERT INTO card (name, cardmarket_id, expansion_id) VALUES ? ON DUPLICATE KEY UPDATE name = VALUES(name)",
      [rows],
    );
    console.log(`${file}: ${rows.length} cards inserted!`);
  }
};
